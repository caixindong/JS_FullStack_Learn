...
export class ReminderContainer extends Component {
  ...
  _done(index) {
    ...
    var currentNumOfItems = this.state.numOfItems;
    if (listData.list[index].selected) {
      currentNumOfItems--;
    } else {
      currentNumOfItems++;
    }
    const numOfItems = currentNumOfItems;
    this.setState({
      listData,
      numOfItems,
    });
    ...
    list.push(
      <View key="add" style={styles.reminderList}>
        <View style={styles.add}>
          <Icon name="md-add" style={styles.addIcon} color="#c6c6c6" size={20}/>
        </View>
        <View style={styles.input}>
        ...
        <View style={styles.reminderContent}>
          <TouchableHighlight underlayColor="transparent" onPress={this.props.switch}>
            <View style={styles.reminderTitleContainer}>
              <Text style={[styles.reminderTitle,{color:listData.theme}]}>{listData.title}
              </Text>
              <Text style={[styles.reminderTitle,{color:listData.theme}]}>{this.state.numOfItems}
              </Text>
            </View>
          </TouchableHighlight>
